...
public interface ItemDetailDao extends GenericDao<ItemDetail, Long>  {
	...
	@Query("select i from ItemDetail i where i.type =?1 and i.category <> ?2 and i.status =?3 and (DATE(i.startDate) <= DATE(?4) or DATE(i.endDate) <= DATE(?4))")
	List<ItemDetail> getByTypeAndCategoryAndStatusAndDate(String type, String category, String status, Date todayDate);
