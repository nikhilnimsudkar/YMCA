...
public class FileItemWriter<T> extends FlatFileItemWriter<T> implements StepExecutionListener, FlatFileFooterCallback, FlatFileHeaderCallback {
	...
	public void beforeStep(StepExecution stepExecution) {
		...
		try {
			...
			log.debug("DEBUG - set the file path as beforeStep"+FILE_PATH);
	...
	public void write(final List<? extends T> items) throws Exception {
		...
		for (int i = 0 ; i < items.size() ; i++) {
			...
			for (FieldMapping m : fieldMappings) { 
					...
					log.debug("DEBUG -inside write method fv obj found as: "+m.getFromField() + " - " +fv);
					...
					if(fv!=null){
						...
						log.debug("DEBUG -inside write method setting fv obj for "+m.getFromField()+" found as: "+fv);
						if(m.getFromField().equalsIgnoreCase(Constants.TRANSACTION_AMOUNT) || m.getFromField().equalsIgnoreCase(Constants.TRANSACTION_AMOUNT1)){
							log.debug("DEBUG -inside write method: "
	...
	public void writeFooter(Writer writer) throws IOException {
		// TODO Auto-generated method stub
		log.debug("DEBUG -Inside writeFooter method..");
		sno = sno + counter - 1;
		...
		if(uploadFile){
			...
			log.debug("DEBUG - file found on footer to upload "+outFile1);
