	...
	<div class="k-loading-image">
		...
		<form:form commandName="signup" class="bootstrap-frm cmxform" id="createDonationForm" method="post" action="donationEmployerMatch" style="padding:20px; max-width: 800px;">
			...
			<div>				 
				  ...
				  <span style="">
					<span class="label-header1">Select Donation :</span>
					<select id="donationItemDetailSelect" name="donationItemDetailLst" style="width : 260px;margin-left : 30px; ">
						  <option value="-1">--Select Donation--</option>
						  <c:forEach var="donation" items="${donationItemDetailLst}" varStatus="count">
								<option value="${donation.id }">${donation.recordName } ${donation.description }</option>
						  </c:forEach>	
				  ...
				  <span id="empMatchDetailsSpan" style="display:none;">
					  ...
					  <span id="donationAmtSpanIdr">
							  ...
							  <input type="text" name="itemDetailId" id="donationTCItemDetailId" value="${donationItemDetailLst[0].id }" style="width: 250px;display : none;">
					  ...
					  <span>
					   ...
					   <div id="purchase_info1" style="margin-left : 18px;">
						...
						<c:if test="${donationItemDetailLst != null && donationItemDetailLst[0] != null && donationItemDetailLst[0].waiversAndTC != null && not empty donationItemDetailLst[0].waiversAndTC.tcDescription && empty agentUid}">
							...
							<span style="display : none;" id="commonTAncHidden">${donationItemDetailLst[0].waiversAndTC.tcDescription  }</span>
						</c:if>
						  <div id="statusBlock">
...
<script>
	...
	$(document).ready(function() {
		...
		$('#donationItemDetailSelect').kendoDropDownList();
		var donationItemDetailSelect = $("#donationItemDetailSelect").data("kendoDropDownList");
		donationItemDetailSelect.select(0);
		
		$(document).on( "change", "#donationItemDetailSelect", function() {			
			if(donationItemDetailSelect.text()=='Other'){					
				$('#donationTCItemDetailId').attr("value","0");
			}else{
				$('#donationTCItemDetailId').attr("value",$(this).val());
			}
		 });
		...
		$("#createDonationButton").click(function(){
			console.log("createDonationButton click");
			 ...
			 if(!$("#createDonationForm").valid()) {
	    		...
	    		<c:if test="${donationItemDetailLst != null && donationItemDetailLst[0] != null && donationItemDetailLst[0].waiversAndTC != null && not empty donationItemDetailLst[0].waiversAndTC.tcDescription}">
