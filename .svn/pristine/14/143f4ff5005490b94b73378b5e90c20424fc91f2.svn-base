...
public class SignUpService extends BaseService {
	...
	public void saveActivity(Account customer, User u, Signup signupprogram) {
		if(signupprogram!=null){
			...
			if(SignupStatusEnum.Active.toString().equalsIgnoreCase(signupprogram.getStatus()))
	...
	public void assignParentSignUp(String parentSignUpItem,Signup signupprogram) {
		try {
			if (StringUtils.isNoneBlank(parentSignUpItem)) {
				...
				Signup parentSignUp =  signupDao.findFirst1ByContactIdAndItemDetailIdAndStatusOrderBySignupIdDesc(Long.parseLong(ids[0]),Long.parseLong(ids[1]), SignupStatusEnum.Active.toString());
	...
	public Signup updateSignup(Signup signup) {
		...
		Signup s = signupDao.save(signup);
		return s;
	}
	
	public Signup signupCapacityManagementForMembershipAndDonation(ItemDetail itemDetail, Signup signup, boolean update) {
		String signupStatus = SignupStatusEnum.Active.toString();
		Long waitListPriority = 0L;
		Integer noofTickets = 1;		
		if(itemDetail!=null){
			Long remainingCapacity = convertNullToZero(itemDetail.getRemainingCapacity());			
			if(signup != null && signup.getNoOfTickets()!=null){
				noofTickets = signup.getNoOfTickets();
			}			
			if(update)
				updateCapacity(itemDetail,noofTickets);			
