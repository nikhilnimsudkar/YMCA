...
<table style="padding-left:4px;padding-top:4px; width: 100%;">
	...
	<tr align="left">
		<td align="left">
			<div style="height:50px; padding-top: 30px; padding-right: 20px;" >
				Please enter the primary contact's First Name and Last Name
				<input id="updateContactHealthHistorySignText" class="k-textbox" style="width: 250px;" placeholder="Full Name" tabIndex="<%= ++tabIndex %>"/>
			</div>
		</td>
	</tr>
	<tr align="left">
...
<script>
	$(document).ready(function() {
		...
		$("#contactHHForm").submit(function(e)
		{
		    ...
		    if(isValid){
		    	var fname_lname;
		    	var signText = $("#updateContactHealthHistorySignText").val();
		    	if(signText && signText != ''){
			    	$.ajax({
						type: "GET",
						url:"getPrimaryUserForAccount",
						dataType: "json",
						async: false,
						success: function( data ) {
							fname_lname = data.fname+" "+data.lname;
							
							console.log("fname_lname "+fname_lname);
					    	console.log("signText "+signText);
					    	if(fname_lname && fname_lname != '' && fname_lname != signText){
					    		$("#tcSuccessSpan1").css("display", "none");
							  	$("#tcSuccessSpan1" ).html("");
							  	$("#tcErrorSpan1").css("display", "block");
							  	$("#tcErrorSpan1" ).html("Please enter the primary contact's full name");
							  	//$("#"+elementId).focus();
							  	return false;
					    	}else{
				  	...
				  	$("#tcSuccessSpan1" ).html("");
				  	$("#tcErrorSpan1").css("display", "block");
				  	$("#tcErrorSpan1" ).html("Please enter the primary contact's First Name and Last Name");
				  	$("#"+elementId).focus();
				  	return false;
		   		}
		    }
