...
<script type="text/x-kendo-template" id="large-cart-item">
            <tr>
                <td>
					...
					<br>
					<span><strong>Instructions:- </strong></span><br>
				...
				<td align="right" width="280px" style="padding-top: 15px;" valign="top">
					...
					#if(promotionMap != undefined && promotionMap.length > 0){#
						#for(var k=0; k<promotionMap.length; k++){#
							#if(promotionMap[k] != null && promotionMap[k] != undefined && promotionMap[k].AutoPromo == 'Yes' && promotionMap[k].PromoRuleType == 'Sign Up' && promotionMap[k].discountValue > 0){#
					...
					#if(promotionMap != undefined && promotionMap.length > 0){#
						#for(var k=0; k<promotionMap.length; k++){#
							#if(promotionMap[k] != null && promotionMap[k] != undefined && promotionMap[k].AutoPromo == 'Yes' && promotionMap[k].PromoRuleType == 'JoinFee' && promotionMap[k].discountValue > 0){#
								<p class="auto_promo">			
					...
					#if(promotionMap != undefined && promotionMap.length > 0){#
						#for(var k=0; k<promotionMap.length; k++){#
							#if(promotionMap[k] != null && promotionMap[k] != undefined && promotionMap[k].AutoPromo == 'Yes' && promotionMap[k].PromoRuleType == 'SetUpFee' && promotionMap[k].discountValue > 0){#
								<p class="auto_promo">			
									<span class="table-price">#= promotionMap[k].PromoCode #</span>
									<span class="cartpricedisplay">
										<span class="discountpricelbl">-</span>
					...
					#if(promotionMap != undefined && promotionMap.length > 0){#
						#for(var k=0; k<promotionMap.length; k++){#
							#if(promotionMap[k] != null && promotionMap[k] != undefined && promotionMap[k].AutoPromo == 'Yes' && promotionMap[k].PromoRuleType == 'Registration' && promotionMap[k].discountValue > 0){#
								<p class="auto_promo">			
									<span class="table-price">#= promotionMap[k].PromoCode #</span>
									<span class="cartpricedisplay">
										<span class="discountpricelbl">-</span>
					...
					#if(promotionMap != undefined && promotionMap.length > 0){#
						#for(var k=0; k<promotionMap.length; k++){#
							#if(promotionMap[k] != null && promotionMap[k] != undefined && promotionMap[k].AutoPromo == 'Yes' && promotionMap[k].PromoRuleType == 'Deposit' && promotionMap[k].discountValue > 0){#
								<p class="auto_promo">			
									<span class="table-price">#= promotionMap[k].PromoCode #</span>
									<span class="cartpricedisplay">
										<span class="discountpricelbl">-</span>
					...
					<p>
						<table>
# if (item.programType == 'Camp' && isMinPayment) { #
<tr>
							<td style="border-bottom: 0px;">
<span class="table-price"><input type="radio" name="AmountRadio_#= uniqueId #" id="AmountRadio1_#= uniqueId #" value="finalAmtMin" onchange="updateCart('#= uniqueId #', 'FinalAmount', $(this))">&nbsp;&nbsp;MINIMUM AMOUNT:</span>								
</td>
<td  align="right" style="border-bottom: 0px;">
	<span class="table-price cartpricedisplay" data-bind="text: finalPriceMinimum"><span>
</td>
							</tr>
# } #
							<tr>
								<td style="border-bottom: 0px;">
									<span class="table-price" width="150px">
# if (item.programType == 'Camp' && isMinPayment) { #
<input type="radio" name="AmountRadio_#= uniqueId #" id="AmountRadio_#= uniqueId #" checked value="finalAmtFull" onchange="updateCart('#= uniqueId #', 'FinalAmount', $(this))">&nbsp;&nbsp;
FULL AMOUNT:
# }else{ #
&nbsp;&nbsp;FINAL AMOUNT:
# } #
...
function processOption(product_id,contact_id){
	//alert();
	if(cart.contents.length>0){
		$.each(cart.contents, function(i,cartItems) {
			var prodId = cartItems.item.prodId;
			var contactId = cartItems.contact.contactId;
			$("#uniform-user_"+prodId+"_"+contactId +" span").attr("class", "");
			$("#user_"+prodId+"_"+contactId).attr('checked', false);
			$("#user_"+prodId+"_"+contactId).attr('disabled', true);
			//$("#user_"+prodId+"_"+contactId).css('disabled', true);
			$("#cname_"+prodId+"_"+contactId).css('color', '#999999');
		});
		
	}
	
	if ($("#user_"+product_id+"_"+contact_id).is(':disabled')){
