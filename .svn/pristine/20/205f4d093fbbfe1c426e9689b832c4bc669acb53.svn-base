...
<span class="head" style="padding-left: 5px;">FUTURE DUE</span>
<table cellpadding="0" cellspacing="0" width="95%" rules="none"	style="padding-left: 4px; padding-top: 4px;">
	<td style="color: red;"><strong><span id="futureDueAmount">$${futureDueAmount}0</span></strong></td>
	</br>
</table>
<br />
<c:choose>
...
<c:if test="${fn:length(donationOpptyLst)>0}">
	<div id="tabstrip4" class="k-block" style=" background-color:#FFFFFF;margin-left:-71px;">
		...
		<br />
		<table cellpadding="0" class="fixedtable" cellspacing="0" width="50%"  rules="none" style="padding-left:4px;padding-top:4px; border : thin 1px solid grey; margin-top : 8px;">					
					...
					<c:if test="${status.count<='1' && opptyRevenue != null && opptyRevenue.itemDetail != null}">															
						<tr style="background-color: #f5f5f5;">	
							<%-- <td style="border: 1px solid white;"><input type="radio" name="oppDonationRadio"><span style="display:none;" class="opptyIdCls">${opportunity.id }</span></td> --%>											
		 					<td align="left"  style="  background-color: #FFFF66;  font-size: 12px; font-weight: bold; color: blue;padding-left:4px;padding:3px; border: 1px solid white; width :45%">
		 						 <div style=""><a href="annualDonationDonate?opptyId=${opportunity.optyId }" >${opptyRevenue.itemDetail.recordName}</a></div>
		 					</td>
		 					<td align="left"  style=" font-size: 10px;color: #006B6B;padding-left:4px;padding:3px; border: 1px solid white; width :45%"> 
		 						${opportunity.name }
		 					</td>
		 				</tr>
					</c:if>	
