...
public interface SignUpDaysDao extends GenericDao<SignUpDays, Long>  {
	...
	@Query("select sd from SignUpDays sd INNER JOIN sd.signup s where sd.status = 'ACTIVE' and s.status IN ('Active', 'Waitlisted') and s.itemDetail.id = ?1 and s.contact.contactId = ?2 ")
