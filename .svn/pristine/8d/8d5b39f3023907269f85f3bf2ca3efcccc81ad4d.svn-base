...
<table style="padding-left:4px;padding-top:4px;" id="currentConfirmed">
	<!-- 
	<tr>
	...
	<tr><td>&nbsp;</td></tr>
	 -->
	<c:choose>
		<c:when test="${fn:length(currentConfirmedProgramsArr)==0}">
			<tr>
				<td colspan="4" align="left">
					<div class="k-block k-error-colored">
						No Current Confirmed Programs
					</div>
				</td>
			</tr>	
		</c:when>
		<c:otherwise>		
			...
			<c:forEach var="program" items="${currentConfirmedProgramsArr}" varStatus="status">					
...
<c:if test="${fn:length(currentConfirmedProgramsArr)>0}">
	<script>
	$(document).ready(function(){ 
		 $("#currentConfirmed").kendoGrid({
	         pageable: true,
	         dataSource: {
	             pageSize: 10
	         }
