...
public class EventController extends BaseController {
    ...
    public ModelAndView eventRegistration(final HttpServletRequest request, final HttpServletResponse response) { 
		...
		model.addAttribute("productcategories", itemDetailsDao.getDistinctTypeForCategory(ProductTypeEnum.EVENT.toString()));
    ...
    public @ResponseBody String  getEventDetails(
		...
		try {
			...
			log.error("Error get event detail ",e);
			return null;
		}
		try {
			/*
			...
			log.error("Error get event detail ",e1);
			return null;
		}
	}
