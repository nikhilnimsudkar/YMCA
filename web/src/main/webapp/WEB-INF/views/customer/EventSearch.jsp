				...
				<span class="searchFields">
					<select name="location" id="location" style="width:250px;">
						<c:forEach var="location" items="${locations}" varStatus="count">
							<c:choose>
								<c:when test="${location.id==usInfo.location.id}">
									<option selected="selected" value="${location.id }">${location.branch }</option>
								</c:when>
								<c:otherwise>
...
<script>
	$(document).ready(function(){
		...
		if($.sessionStorage.getItem("search_event_filter") != null){
			var filterJson = JSON.parse($.sessionStorage.getItem('search_event_filter'));
