...
<table style="padding-left:4px;padding-top:4px;" id="currentWaitlisted">
	<!-- 
	<tr>
	...
	<c:choose>
		<c:when test="${fn:length(currentWaitlistedProgramsArr)==0}">
			<tr>
				<td colspan="4" align="left">
					<div class="k-block k-error-colored">
						No Current Wait listed Programs
					</div>
				</td> 
			</tr>	
		</c:when>
		<c:otherwise>
			...
			<c:forEach var="program" items="${currentWaitlistedProgramsArr}" varStatus="status">					
				<tr class="troutline k-block">
					...
					<%--</c:if> --%>
					<td>
						<c:if test="${sessionScope.agentUid != null && program.actualRemainingCapacity > 0 }">
							<a href="#" onclick="confirmProgram(${program.signupId})">Confirm</a>
						</c:if>
						<c:choose>
...
<c:if test="${fn:length(currentWaitlistedProgramsArr)>0}">
	<script>
	$(document).ready(function(){ 
		 $("#currentWaitlisted").kendoGrid({
	         pageable: true,
	         dataSource: {
	             pageSize: 10
	         }
