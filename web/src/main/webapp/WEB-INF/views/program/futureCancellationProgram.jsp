...
<table style="padding-left:4px;padding-top:4px;" id="futureCancellation">
	
	<c:choose>
		<c:when test="${fn:length(futureCancelledProgramsArr)==0}">
			<tr>
				<td colspan="4" align="left">
					<div class="k-block k-error-colored">
						No Future Cancellation Programs
					</div>
				</td>
			</tr>	
		</c:when>
		<c:otherwise>		
			...
			<c:forEach var="program" items="${futureCancelledProgramsArr}" varStatus="status">					
...
<c:if test="${fn:length(futureCancelledProgramsArr)>0}">
	<script>
	$(document).ready(function(){ 
		 $("#futureCancellation").kendoGrid({
	         pageable: true,
	         dataSource: {
	             pageSize: 10
	         }
