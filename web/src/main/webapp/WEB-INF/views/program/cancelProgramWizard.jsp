		...
		<div id="programs">
			<div id="programinfoPg">
				...
				<div id="tabstrip3" class="k-block" style=" background-color:#FFFFFF;margin-left:-52px;width:825px;min-height:300px; padding: 5px 5px 25px;">
						...
						<c:choose>
							<c:when test="${fn:length(enrolledProgramDetailsArr)==0}">
								...
								<c:forEach var="e" items="${errorMessageArr}" varStatus="status">
									<div class="k-block k-error-colored">${e.errMsg }</div><br><br>
									<span class='k-button' id='backtoviewprogram'><< Back</span>
								</c:forEach>
							</c:when>
							<c:otherwise>
								<c:forEach var="program" items="${enrolledProgramDetailsArr}" varStatus="status">
									...
									<div id="programdetails" style="padding: 0px 0px 22px 30px; margin:12px;" class="k-block k-shadow">
										...
										<div id="sStatus">
											...
											<input type="hidden" id="SignupStatus" name="SignupStatus" value="${program.programEnrollmentStatus }">
										</div>
					...
					<br>
					<span class='k-button' id='backtoviewprogram' style="display:none;"><< Back</span>
				</div>
			</div>
		</div>
...
<script>
	$(document).ready(function(){ 
		...
		$("#datepickerFutureCancelDate").kendoDatePicker({
			value: new Date(new Date().getTime() + 24 * 14 * 60 * 60 * 1000),
			format: "MM/dd/yyyy",
			min: new Date()
		});
		...
		$( "#backtoviewprogram" ).click(function(){
			location.href = 'viewAllPrograms';
		});
